...
1. tuple前面加*号，可以变成可变参数传进去，这个是链接，还是没怎么懂https://zhidao.baidu.com/question/369701615352347164.html
...
2. numpy矩阵的乘法：
   ...
   - 区分好几个名词，点积=内积，适用于向量，而矩阵乘法是适用于矩阵，实质也还是矩阵行列之间的点积，矩阵的元素相乘和这些都没关系

   - 二维时**np.matmul()**和**np.dot()**等价，而**np.multiply()**和*****实现了元素级别乘法，参考链接：https://blog.csdn.net/u012300744/article/details/80423135
   - 关于**np.matmul()**和**np.dot()**的区别，要详细看numpy文档（暂时没看），以下链接是别人写的，可以参考下链接：https://blog.csdn.net/qq_42698384/article/details/82936294

3. numpy矩阵的sum：记得axis=谁，谁那一轴就消失，比如dout的维度是(10,5),做np.sum(dout,axis=0)，那么结果的维度就是（5，）

4. python默认向量维度是列向量；

   - numpy的一维列向量维度是（n,），二维横向量的维度是（1,n）

   - 一维列向量（如(3,)）在广播运算中是当做二维行向量（如(1,3)）计算的，也就是说(3,)相当于(1,3)

5. 矩阵乘法计算时注意维度的问题呀，是真滴还是计算时很杂乱，有机会去补一下这个基础哇

6. 对于db的求导是真滴没搞懂，本质上是张量的乘法吗？这个需要问一下老师🌟有一个参考链接，但是没怎么看懂，http://tieba.baidu.com/p/4139437334

7. 另附上一个矩阵相关求导的公式，附上链接，以后看https://blog.csdn.net/max_hope/article/details/80264229
...
这里也是给了以前完成的部分，但是还是值得按照给的标准答案进行分析每一步的trick
...
2. `np.zeros_like(x)`: 类似`np.zeros(x.shape)`

3. 对svm的求导公式一直不是记忆得很牢哈，简单做个总结：

   - 首先令margin为0的部分为0，不参与求导

     `dx = np.zeros_like(x) # (N,C)`

   - 而margin>0的部分需要考虑两个部分，一个是`xi`,一个是`xy`,

     即`dx[margins > 0] = 1 # (N,C)`考虑每一个xi求导都是1

   - 而因为`xy`每一次求导时都要得出一个**-1**，所以需要求出所有margin>0的数量num，在最后减去num

     即`num_pos = np.sum(margins > 0, axis=1) # (N,)`
...
1. 列表推导式：
   ...
   - `np.max(x, axis=1, keepdims=True)`：
     ...
     不加`keepdims=True`时，维度由(N,C)变成了(N,)，为了保持维度的数目不变，加上后变成了(N,1)

   - 悄悄附上链接，这个总结以后可以常去看，总结numpy维度的问题https://www.jianshu.com/p/2adbf3a44a95
...
2. 给字典对象添加新的对象：
   ...
   例子：`self.params['W1'] = W1`

   更多操作见链接：http://www.cnblogs.com/scios/p/8108243.html

3. loss函数中值得关注的点：

   正则化是针对于权重系数的（不包括偏置）？？（有些不太懂）

   loss的正则化用的是L2正则，而grads的正则是L1正则
